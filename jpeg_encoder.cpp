...
#pragma pack(pop)
	...
	do
	{
		...
		if (1 != fread(&fileHeader, sizeof(fileHeader), 1, fp)) break; //读取文件头
		if (fileHeader.bfType != 0x4D42) break; //判断是否为BMP图片
		if (1 != fread(&infoHeader, sizeof(infoHeader), 1, fp)) break; //读取位图
		if (infoHeader.biBitCount != 24 || infoHeader.biCompression != 0) break;//是否为24位位图
		int width = infoHeader.biWidth; //获取图片的宽
		int height = infoHeader.biHeight < 0 ? (-infoHeader.biHeight) : infoHeader.biHeight;//获取图片的高
